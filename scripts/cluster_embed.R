...
cluster_align <- function(distances,
                          clusters,
                          embedding_method = c("PCA", "NA"),
                          alignment_method = c("stress", "s-stress", "sammon"), # functionality not completely implemented
                          alpha = 1,
  ...
  embedding_obj$translations <- initialize_translations(distances, clusters, method = "PCA")
...
get_alpha <- function(distances, emb_distances, clusters, q) {
  ...
  for (i in 1:n_clusters) {
    ...
    diams <- c(diams, quantile(emb_distances[cluster_i, cluster_i], q)) # more robust to use i.e. 95th quantile of pairwise distances for diameter
  }
  bw <- c()
  for (i in 1:(n_clusters - 1)) {
    ...
    for (j in (i + 1):n_clusters) {
      ...
      bw <- c(bw, mean(distances[cluster_i, cluster_j]))
    }
  }
  return(mean(diams) * n_clusters / (2 * pi * mean(bw)))
}
...
embed_clusters <- function(distances, clusters, embedding_method) {
  ...
  # TO DO: add other embedding methods here
...
initialize_translations <- function(distances, clusters, method = c("PCA", "random")) {
...
# reformat embedding object so ready for visualization
